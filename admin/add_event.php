...
error_reporting(1);
   ...
   $GetDetailByid=$driverObj->GetDetailsByid(base64_decode($_GET['eid']),'tbl_newroom');
  
   $employee_data=$driverObj->getalltable1('tbl_newroom','2');
   //echo "<pre>"; print_r($employee_data);die; 
if(isset($_GET["did"]))
{
    $result=$med_common_Obj->DeleteDataById($_GET["did"],'tbl_newroom');
    if($result>0)
    {
      header("Location:add_event.php?msg=ds");
    }
    else
    {
      header("Location:add_event.php?msg=df");
    }
}

 if(isset($_POST["Add"]))
   {
     echo  $result=$driverObj->Addnewroom($_POST);
       if($result>0)
       {
         header("Location:add_event.php?msg=ins");
       }
       else
       {
         header("Location:add_event.php?msg=inf");
       }
   }
 
    if(isset($_POST["update"]))
   {
       $result=$driverObj->upnewsroom($_POST);
       if($result>0)
       {
         header("Location:add_event.php?msg=ups");
       }
       else
       {
         header("Location:add_event.php?msg=upf");
       }
...
<label for="vehicle_make" class="">Title</label> 
							...
							<input type="hidden" name="newroom" value="2" class="form-control">
											  </div>

</div>

<div class="col-md-3">
<label for="vehicle_make" class="">Url</label> 

                                             <div class="input-group">
							<input type="text" name="url" value="<?=$GetDetailByid['link']?>" class="form-control">
 ...
 <div class="card">
                                ...
                                <div class="main-card mb-3 card">
                                    ...
                                    <div class="card-body"><h5 class="card-title">View All Event
...
<td>
   ...
   <script type="text/javascript">
...

      {
        ...
        window.location.href='add_event.php?eid='+id
...

{
  ...
  window.location.href='add_event.php?did='+id
...

{
  ...
  window.location.href='add_event.php?a_id='+window.btoa(id) + "&val=" + window.btoa(value);
